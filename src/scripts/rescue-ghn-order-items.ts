...
async function getOrderByCode(code: string) {
  const rows = await prisma.$queryRawUnsafe<any[]>(
    `SELECT * FROM "Order" WHERE code = $1 LIMIT 1`,
    code
  );
  return rows[0] || null;
}
...
async function findBranchFallback() {
  const rows = await prisma.$queryRawUnsafe<any[]>(
    `
      ...
      ORDER BY
        CASE
          WHEN lower(coalesce(code,'')) = 'qo' THEN 0
          WHEN lower(coalesce(name,'')) LIKE '%quốc oai%' THEN 1
          WHEN lower(coalesce(name,'')) LIKE '%quoc oai%' THEN 1
          ELSE 2
        END
      LIMIT 1
    `
