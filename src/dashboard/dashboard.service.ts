...
export class DashboardService {
  ...
  async getOverview(branchId?: string) {
    ...
    const orderWhere: any = selectedBranchId
      ? { branchId: selectedBranchId }
      : {};
    const validOrderWhere: any = {
      ...orderWhere,
    ...
    const [
        ...
        .count({
          where: {
            ...(selectedBranchId
              ...
              : {}),
            status: {
              in: ["DRAFT", "PENDING", "CONFIRMED", "IN_TRANSIT"] as any,
            },
          },
        })
        .catch(() => 0),
      ...
      this.prisma.orderItem.groupBy({
        ...
        where: {
          ...
          order: {
            ...validOrderWhere,
            soldAt: { gte: last7Start, lt: tomorrowStart },
          },
        },
        _sum: { qty: true, lineTotal: true },
      }),
      this.prisma.orderItem.groupBy({
        ...
        where: {
          ...
          order: {
            ...validOrderWhere,
            soldAt: { gte: last14Start, lt: tomorrowStart },
          },
        },
        _sum: { qty: true, lineTotal: true },
      }),
    ...
    const sold7Map = new Map(
      sales7Raw.map((row) => [
        row.variantId,
        { qty: this.n(row._sum.qty), revenue: this.n(row._sum.lineTotal) },
      ]),
    );
    const sold14Map = new Map(
      sales14Raw.map((row) => [
        row.variantId,
        { qty: this.n(row._sum.qty), revenue: this.n(row._sum.lineTotal) },
      ]),
    );
    const sold30Map = new Map(
      sales30Raw.map((row) => [
        row.variantId,
        { qty: this.n(row._sum.qty), revenue: this.n(row._sum.lineTotal) },
      ]),
    ...
    const candidateVariantIds = Array.from(
      new Set([
        ...variantStockGroups
          .filter((row) => this.n(row._sum.availableQty) <= 3)
    ...
    const criticalCandidates: CriticalSku[] = candidateVariants
      .map((variant) => {
        const stockInfo = stockByVariant.get(variant.id) || {
          stock: 0,
          reserved: 0,
          incoming: 0,
        };
    ...
    const lowStockItems = criticalCandidates.filter(
      (item) => item.stock > 0 && item.stock <= 3,
    ).length;
    const outOfStockItems = criticalCandidates.filter(
      (item) => item.stock <= 0,
    ).length;
    ...
    const totalWarehouseQty = warehouseGroups.reduce(
      (sum, row) => sum + this.n(row._sum.availableQty),
      0,
    );
    const warehouseMix = warehouseGroups
      .map((row) => {
        ...
        const percent =
          totalWarehouseQty > 0
            ? Math.round((qty / totalWarehouseQty) * 100)
            : 0;
        const level =
          percent >= 45
            ? "Kho đang giữ tỷ trọng lớn"
            : percent <= 10
              ? "Kho tỷ trọng thấp"
              : "Tỷ trọng ổn";
        return {
    ...
    const topProducts = topProductsRaw.map((row, index) => {
      ...
      const stock = row.variantId
        ? stockByVariant.get(row.variantId)?.stock || 0
        : 0;
    ...
    const maxChannelRevenue = Math.max(
      ...channelRevenueRaw.map((row) => this.n(row._sum.finalAmount)),
      1,
    );
    ...
    // Ép merge ads vào dailyMap sau khi cộng đơn hàng. Đây là điểm fix chính:
    // totalAdsCost có số thì dailyRows từng ngày cũng phải nhận đúng adsCost.
    for (const [key, row] of dailyMap.entries()) {
      const liveAdsCost = getAdsCostForDay(key);
      row.adsCost = liveAdsCost;
      row.profit = row.revenue - row.cost - liveAdsCost;
      dailyMap.set(key, row);
    }
    ...
    const dailyRows = days.map(([key, row], index) => {
      ...
      const liveAdsCost = getAdsCostForDay(key) || row.adsCost || 0;
      row.adsCost = liveAdsCost;
      ...
      const roas =
        row.adsCost > 0
          ? `${(row.revenue / row.adsCost).toFixed(2)}x`
          : "Chưa nối ads";
      return {
        day,
        date: key,
        displayDate: `${day}/${String(date.getMonth() + 1).padStart(2, "0")}/${date.getFullYear()}`,
    ...
    const todayOrders = monthOrders.filter(
      (order) =>
        this.dayKey(order.soldAt || order.createdAt) ===
        this.dayKey(todayStart),
    ).length;
    const checkoutPurchase =
      todayOrders > 0
        ? `${todayCompletedOrders}/${todayOrders}`
    ...
    for (const item of criticalCandidates.slice(0, 4)) {
      ...
      const daysText =
        item.daysToOut !== null && item.daysToOut < 999
          ? ` · dự kiến hết sau ${Math.max(1, Math.ceil(item.daysToOut))} ngày`
          : "";
      decisionCards.push({
    ...
    for (const item of reorderCandidates) {
      ...
      const suggestedQty = Math.max(
        3,
        Math.ceil(
          item.velocity14 * targetCoverDays - item.stock - item.incoming,
        ),
      );
    ...
    if (transferSuggestion) {
      const source = [...transferSuggestion.branchStocks].sort(
        (a, b) => b.availableQty - a.availableQty,
      )[0];
      const target = [...transferSuggestion.branchStocks].sort(
        (a, b) => a.availableQty - b.availableQty,
      )[0];
    ...
    const sortedDecisionCards = decisionCards
      .sort((a, b) => this.n(b.score) - this.n(a.score))
      .slice(0, 10);
    ...
    const systemTone: Tone = sortedDecisionCards.some(
      (c) => c.tone === "critical",
    )
      ? "critical"
      : sortedDecisionCards.some((c) => c.tone === "warning")
        ? "warning"
        : "safe";
    const statusTitle =
      systemTone === "safe"
    ...
    return {
      ...
      hero: {
        ...
        chips: [
          `${this.qty(totalOrders)} đơn`,
          `${this.money(revenue)} doanh thu`,
          `${this.qty(criticalCandidates.length)} SKU critical`,
        ],
        autoMode: "SEMI",
        metaMode: metaAdsConnected ? "LIVE" : "DISCONNECTED",
        metaAccount: metaAdsConnected
          ? "Meta Ads · Nam Nguyen"
          : "Meta Ads chưa nối live data",
      ...
      warningSummary: {
        ...
        roas:
          totalAdsCost > 0
            ? `${(revenue / totalAdsCost).toFixed(2)}x`
            : "Chưa nối Meta",
        inventory:
      ...
      cards: {
        ...
        totalCost,
        totalAdsCost,
        metaAdsCampaignCount: adsRows.length,
        totalOrders,
        newOrders,
        completedOrders,
        cancelledOrders,
      ...
      kpis: [
        ...
        },
        {
          id: "k2",
          label: "Đơn hàng",
          value: this.qty(totalOrders),
          delta: `${this.qty(newOrders)} mới`,
        },
        {
          id: "k3",
          label: "Tồn khả dụng",
          value: this.qty(availableQty),
          delta: `${this.qty(reservedQty)} giữ`,
        },
        {
          id: "k4",
          label: "Top critical SKU",
          value: this.qty(criticalCandidates.length),
          delta: `${this.qty(rawLowStockPool)} pool`,
        },
        {
          id: "k5",
          label: "Lợi nhuận",
          value: hasCostData ? this.money(profit) : "—",
          delta: hasCostData ? "Sau giá vốn + ads" : "Thiếu giá vốn",
        },
      ],
      dailyRows,
      drilldown: [
        ...
        {
          label: "Hoàn thành",
          value: this.qty(todayCompletedOrders || completedOrders),
          tone: "dark",
        },
        {
          label: "Lợi nhuận",
      ...
      quickInsights: [
        ...
        highestVelocity
          ...
          : "Chưa có SKU bán nhanh chạm ngưỡng tồn thấp.",
        topWarehouse
          ? `Kho có tỷ trọng lớn nhất: ${topWarehouse.name} (${topWarehouse.value}).`
          : "Chưa có dữ liệu phân bổ kho.",
        bestChannel
          ? `Kênh doanh thu nổi bật: ${bestChannel.name} (${bestChannel.value}).`
      ...
      funnel: [
        ...
        {
          label: "New",
          value: this.qty(newOrders),
          width:
            totalOrders > 0
              ? `${Math.max(4, Math.round((newOrders / totalOrders) * 100))}%`
              : "4%",
        },
        {
          label: "Completed",
          value: this.qty(completedOrders),
          width:
            totalOrders > 0
              ? `${Math.max(4, Math.round((completedOrders / totalOrders) * 100))}%`
              : "4%",
        },
        {
          label: "Cancelled",
          value: this.qty(cancelledOrders),
          width:
            totalOrders > 0
              ? `${Math.max(4, Math.round((cancelledOrders / totalOrders) * 100))}%`
              : "4%",
        },
      ],
      floatingApproval: {
