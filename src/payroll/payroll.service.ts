...
export class PayrollService {
  ...
  private scopedBranchId(user?: AnyUser, branchId?: string | null) {
    ...
    if (
      branchId &&
      branchId !== "ALL" &&
      String(branchId) !== String(userBranch)
    ) {
      throw new ForbiddenException(
        "Không có quyền xem hoặc thao tác dữ liệu chi nhánh khác.",
      );
    }
    return userBranch;
  }
  ...
  private calcLineTotals(line: PayrollLikeLine) {
    ...
    const netPay =
      grossPay - this.toNumber(line.advance) - this.toNumber(line.deduction);
    return {
  ...
  private isOrderSuccessForPayroll(order: any) {
    ...
    const shipmentStatus = String(
      order?.shipment?.shippingStatus || order?.shipment?.partnerStatus || "",
    ).toUpperCase();
    ...
    if (
      ["DELIVERED", "COMPLETED", "SUCCESS", "FULFILLED"].some((key) =>
        shipmentStatus.includes(key),
      )
    )
      return true;
    return false;
  ...
  private channelEnabled(config: any, order: any) {
    ...
    const isCod =
      String(shipment?.carrier || "").trim() ||
      order?.paymentStatus === PaymentStatus.PENDING_COD;

    if (channel === "POS") return config.applyPos !== false;
    if (channel === "FACEBOOK_MANUAL" || source.includes("FACEBOOK"))
      return config.applyFacebook !== false;
  ...
  async createPeriod(body: CreatePayrollPeriodDto, user?: AnyUser) {
    ...
    const branchName =
      body.branchName || (await this.resolveBranchName(branchId));
    const code = String(
      body.code || this.buildPeriodCode(fromDate, toDate, branchId),
    ).trim();
    const name = String(
      body.name ||
        `Kỳ lương ${fromDate.toISOString().slice(0, 10)} → ${toDate.toISOString().slice(0, 10)}${branchName ? ` · ${branchName}` : ""}`,
    ).trim();
  ...
  async listConfigs(query: any, user?: AnyUser) {
    ...
    if (query.isActive !== undefined && query.isActive !== "ALL")
      where.isActive = String(query.isActive) !== "false";
    if (query.q?.trim()) {
      const q = query.q.trim();
      where.OR = [
    ...
    return this.prisma.payrollConfig.findMany({
      where,
      orderBy: [{ isActive: "desc" }, { staffName: "asc" }],
    });
  }
  ...
  async updateConfig(id: string, body: PayrollConfigDto, user?: AnyUser) {
    const current = await this.prisma.payrollConfig.findUnique({
      where: { id },
    });
      ...
      data: {
        ...
        baseSalary:
          body.baseSalary === undefined
            ? undefined
            : this.money(body.baseSalary),
        dailyRate:
          body.dailyRate === undefined ? undefined : this.money(body.dailyRate),
        standardWorkingDays:
          body.standardWorkingDays === undefined
            ? undefined
            : this.decimal2(body.standardWorkingDays),
        commissionPerOrderEnabled: body.commissionPerOrderEnabled ?? undefined,
        commissionPerOrderAmount:
          body.commissionPerOrderAmount === undefined
            ? undefined
            : this.money(body.commissionPerOrderAmount),
        commissionPerItemEnabled: body.commissionPerItemEnabled ?? undefined,
        commissionPerItemAmount:
          body.commissionPerItemAmount === undefined
            ? undefined
            : this.money(body.commissionPerItemAmount),
        commissionPercentEnabled: body.commissionPercentEnabled ?? undefined,
        commissionRate:
          body.commissionRate === undefined
            ? undefined
            : this.decimal2(body.commissionRate),
        ...
        allowanceDefault:
          body.allowanceDefault === undefined
            ? undefined
            : this.money(body.allowanceDefault),
        effectiveFrom: body.effectiveFrom
          ? this.startOfDate(body.effectiveFrom)
          : undefined,
        effectiveTo:
          body.effectiveTo === undefined
            ? undefined
            : body.effectiveTo
              ? this.endOfDate(body.effectiveTo)
              : null,
  ...
  private async activeConfigsForPeriod(period: any) {
    ...
    if (period.branchId)
      where.AND = [{ OR: [{ branchId: period.branchId }, { branchId: null }] }];
    return this.prisma.payrollConfig.findMany({
      where,
      orderBy: [{ branchName: "asc" }, { staffName: "asc" }],
    });
  ...
  async calculatePeriod(
    id: string,
    body: { workingDaysByStaff?: Record<string, number>; force?: boolean } = {},
    user?: AnyUser,
  ) {
    const period = await this.prisma.payrollPeriod.findUnique({
      where: { id },
    ...
    if (["LOCKED", "PAID", "PARTIALLY_PAID"].includes(status) && !body.force) {
      throw new BadRequestException(
        "Kỳ lương đã khóa hoặc đã trả, không thể tính lại.",
      );
    }
    ...
    if (!configs.length)
      throw new BadRequestException(
        "Chưa có cấu hình lương nào phù hợp kỳ này.",
      );
    ...
    await this.prisma.$transaction(
      async (tx) => {
        ...
        for (const config of configs) {
          ...
          const lineBranchId =
            period.branchId || config.branchId || staff.branchId || null;
          const lineBranchName =
            period.branchName || config.branchName || staff.branchName || null;
          const workingDays = this.toNumber(
            body.workingDaysByStaff?.[config.staffId] ??
              config.standardWorkingDays ??
              26,
          );
          const standardDays = Math.max(
            1,
            this.toNumber(config.standardWorkingDays || 26),
          );
          ...
          const proratedSalary =
            salaryType === "NONE"
              ? 0
              : salaryType === "DAILY" || salaryType === "SHIFT"
                ? dailyRate * workingDays
                : (baseSalary * Math.min(workingDays, standardDays)) /
                  standardDays;

          const orderWhere: Prisma.OrderWhereInput = {
            createdByStaffId: config.staffId,
            createdAt: { gte: period.fromDate, lte: period.toDate },
            status: { not: OrderStatus.CANCELLED },
          };
          if (lineBranchId) orderWhere.branchId = lineBranchId;
          ...
          const validOrders = rawOrders.filter(
            (order: any) =>
              this.isOrderSuccessForPayroll(order) &&
              this.channelEnabled(config, order),
          );
          const successOrderCount = validOrders.length;
          const successItemQty = validOrders.reduce(
            (sum: number, order: any) => sum + this.orderItemQty(order),
            0,
          );
          const revenueAmount = validOrders.reduce(
            (sum: number, order: any) => sum + this.toNumber(order.finalAmount),
            0,
          );
          ...
          const commissionTotal =
            commissionByOrder + commissionByItem + commissionByPercent;
          ...
          if (validOrders.length) {
            await tx.payrollOrderLink.createMany({
              data: validOrders.map((order: any) => {
                ...
                const byOrder = config.commissionPerOrderEnabled
                  ? this.toNumber(config.commissionPerOrderAmount)
                  : 0;
                const byItem = config.commissionPerItemEnabled
                  ? itemQty * this.toNumber(config.commissionPerItemAmount)
                  : 0;
                const byPercent = config.commissionPercentEnabled
                  ? (revenue * this.toNumber(config.commissionRate)) / 100
                  : 0;
                return {
  ...
  async lockPeriod(id: string, user?: AnyUser) {
    ...
    if (!period) throw new NotFoundException("Không tìm thấy kỳ lương.");
    this.scopedBranchId(user, period.branchId || null);
    if (!Number(period.totalStaff || 0))
      throw new BadRequestException("Kỳ lương chưa có dữ liệu tính lương.");
    return this.prisma.payrollPeriod.update({
      ...
      data: {
        ...
        lockedByName: this.userName(user),
      },
    });
  }
  ...
  async unlockPeriod(id: string, user?: AnyUser) {
    ...
    if (!period) throw new NotFoundException("Không tìm thấy kỳ lương.");
    this.scopedBranchId(user, period.branchId || null);
    if (String(period.status).toUpperCase() === "PAID")
      throw new BadRequestException("Kỳ lương đã trả không thể mở khóa.");
    return this.prisma.payrollPeriod.update({
      where: { id },
      data: {
        status: "CALCULATED",
  ...
  async updateLine(id: string, body: any, user?: AnyUser) {
    ...
    if (["PAID"].includes(String(line.status || "").toUpperCase()))
      throw new BadRequestException("Dòng lương đã trả không thể sửa.");

    const next: any = {
      workingDays:
        body.workingDays === undefined
          ? line.workingDays
          : this.decimal2(body.workingDays),
      bonus: body.bonus === undefined ? line.bonus : this.money(body.bonus),
      allowance:
        body.allowance === undefined
          ? line.allowance
          : this.money(body.allowance),
      advance:
        body.advance === undefined ? line.advance : this.money(body.advance),
      deduction:
        body.deduction === undefined
          ? line.deduction
          : this.money(body.deduction),
  ...
  async addAdjustment(id: string, body: PayrollAdjustmentDto, user?: AnyUser) {
    ...
    const data: any = {};
    if (type === "BONUS")
      data.bonus = this.money(this.toNumber(line.bonus) + amount);
    if (type === "ALLOWANCE")
      data.allowance = this.money(this.toNumber(line.allowance) + amount);
    if (type === "ADVANCE")
      data.advance = this.money(this.toNumber(line.advance) + amount);
    if (type === "DEDUCTION")
      data.deduction = this.money(this.toNumber(line.deduction) + amount);
  ...
  private async recalculatePeriodTotals(periodId: string) {
    ...
    const totalOrders = lines.reduce(
      (s, l) => s + Number(l.successOrderCount || 0),
      0,
    );
    const totalItems = lines.reduce(
      (s, l) => s + Number(l.successItemQty || 0),
      0,
    );
    const totalRevenue = lines.reduce(
      (s, l) => s + this.toNumber(l.revenueAmount),
      0,
    );
    ...
    const totalPaid = lines.reduce(
      (s, l) => s + this.toNumber(l.paidAmount),
      0,
    );
  ...
  async markLinePaid(
    id: string,
    body: { paymentSourceId?: string; paidAmount?: number; note?: string },
    user?: AnyUser,
  ) {
    const line = await this.prisma.payrollLine.findUnique({
      where: { id },
    ...
    const paidAmount = Math.min(
      this.toNumber(body.paidAmount || line.netPay),
      this.toNumber(line.netPay),
    );
    if (paidAmount <= 0)
      throw new BadRequestException("Số tiền trả phải lớn hơn 0.");
    ...
    if (body.paymentSourceId) {
      ...
      voucher = await this.financeService.confirmCashVoucher(
        voucher.id,
        {},
        user,
      );
    ...
    const updated = await this.prisma.payrollLine.update({
      ...
      data: {
        status:
          paidAmount >= this.toNumber(line.netPay) ? "PAID" : "PARTIALLY_PAID",
  ...
  async markPeriodPaid(
    id: string,
    body: { paymentSourceId?: string; note?: string },
    user?: AnyUser,
  ) {
    const period = await this.prisma.payrollPeriod.findUnique({
      where: { id },
    ...
    if (!period) throw new NotFoundException("Không tìm thấy kỳ lương.");
    this.scopedBranchId(user, period.branchId || null);
    const unpaidLines = period.lines.filter(
      (line) => String(line.status || "").toUpperCase() !== "PAID",
    );
    if (!unpaidLines.length) return period;

    const totalNet = unpaidLines.reduce(
      (sum, line) => sum + this.toNumber(line.netPay),
      0,
    );
    if (totalNet <= 0)
      throw new BadRequestException("Kỳ lương không có số tiền cần trả.");
